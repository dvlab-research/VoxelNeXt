...
class Argo2Dataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        #a = pickle.load(open("../result.pkl", 'rb'))
        #self.evaluation(a, class_names)
    ...
    def __getitem__(self, index):
        ...
        if 'annos' in info:
            ...
            #gt_bboxes_3d = np.concatenate([loc, l, w, h, -(np.pi / 2 + rots[..., np.newaxis])], axis=1)

            gt_bboxes_3d = np.concatenate([loc, dims, rots[..., np.newaxis]], axis=1).astype(np.float32)
            #gt_bboxes_3d[:, 2] -= gt_bboxes_3d[:, 5] * 0.5
