...
class FocalHeadnuScenes(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
        ...
        self.gaussian_type = self.model_cfg.get('GAUSSIAN_TYPE', ['nearst', 'gt_center']) # 'onlyone' 
        self.debug = self.model_cfg.get('DEBUG', False) 
        self.radius_set = self.model_cfg.get('RADIUS_SET', -1)

        self.double_flip = self.model_cfg.get('DOUBLE_FLIP', False)
        self.noise_rot = self.model_cfg.get('NOISE_ROT', 0) * -1
        ...
        shared_conv_func = spconv.SubMConv3d if shared_conv_type=='subm' else spconv.SparseConv3d

        self.shared_conv = spconv.SparseSequential(
            shared_conv_func(
                input_channels, self.model_cfg.SHARED_CONV_CHANNEL, kernel_size_shared, stride=1, padding=int(kernel_size_shared//2),
                bias=self.model_cfg.get('USE_BIAS_BEFORE_NORM', False)
            ),
            nn.BatchNorm1d(self.model_cfg.SHARED_CONV_CHANNEL),
            nn.ReLU(True),
        )
        ...
        for idx, cur_class_names in enumerate(self.class_names_each_head):
            ...
            for bs_idx in range(batch_size):
                ...
                heatmap, ret_boxes, inds, mask = self.assign_target_of_single_head(
                    ...
                    #positive_voxels_gt_inds=positive_voxels_gt_inds[bs_idx], 
                    #positive_voxels_3d=positive_voxels_3d[bs_idx],
                )
    ...
    def distance(self, voxel_indices, center_xyz):
        distances = ((voxel_indices - center_xyz.unsqueeze(0))**2).sum(-1)
        return distances
    ...
    def assign_target_of_single_head(
        ...
        center_xyz = torch.cat((coord_x[:, None], coord_y[:, None], coord_z[:, None]), dim=-1)
        ...
        for k in range(min(num_max_objs, gt_boxes.shape[0])):
            ...
            distance = self.distance(spatial_indices, center_xyz[k])
            ...
            if 'nearst' in self.gaussian_type:
                centernet_utils.draw_gaussian_to_heatmap_voxels(heatmap[cur_class_id], self.distance(spatial_indices, spatial_indices[inds[k]]), radius[k].item() * self.gaussian_ratio if self.radius_set<0 else self.radius_set)

            if 'onlyone' in self.gaussian_type:
                heatmap[cur_class_id][inds[k]] = 1
    ...
    def combine_out(self, x_conv1, x_conv2, ratio=1):
        ...
        x_conv2.spatial_shape = [s*ratio for s in x_conv2.spatial_shape]
    ...
    def merge_double_flip(self, pred_dict, batch_size, voxel_indices, spatial_shape):
        ...
        for bs_idx in range(batch_size):
            ...
            for i in range(4):
                ...
                if i in [2, 3]:
                    spatial_indices[bs_indices, 2] = spatial_shape[2] - spatial_indices[bs_indices, 2]
        ...
        return batch_hm, batch_center, batch_center_z, batch_dim, batch_rot_cos, batch_rot_sin, batch_vel, voxel_indices_

    def noise_rot_boxes(self, pred_boxes):
        if self.noise_rot != 0:
            noise_rotation = torch.Tensor([self.noise_rot]).to(pred_boxes.device)
            pred_boxes[:, 0:3] = rotate_points_along_z(pred_boxes[:, 0:3].unsqueeze(0), noise_rotation)[0]
            pred_boxes[:, 6] += self.noise_rot
            if pred_boxes.shape[1] > 7:
                pred_boxes[:, 7:9] = rotate_points_along_z(
                    torch.hstack((pred_boxes[:, 7:9], pred_boxes.new_zeros((pred_boxes.shape[0], 1)))).unsqueeze(0),
                    noise_rotation
                )[0][:, 0:2]
        return pred_boxes
    ...
    def generate_predicted_boxes(self, batch_size, pred_dicts, voxel_indices, spatial_shape):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            if self.double_flip:
                batch_hm, batch_center, batch_center_z, batch_dim, batch_rot_cos, batch_rot_sin, batch_vel, voxel_indices_ = \
        ...
        for k in range(batch_size):
            ret_dict[k]['pred_boxes'] = self.noise_rot_boxes(torch.cat(ret_dict[k]['pred_boxes'], dim=0))
    ...
    def forward(self, data_dict):
        x = self.shared_conv(data_dict['encoded_spconv_tensor'])
